...
public sealed class PaginatedList<T> : List<T>
{
	...
	public static PaginatedList<T> CreateInstance(IEnumerable<T> source, int page, int quantity)
	{
		...
		List<T> items = source.Skip((page - 1) * quantity).Take(quantity).ToList();

		return new PaginatedList<T>(page, quantity, total, items);
	}

	public static async Task<PaginatedList<T>> CreateInstanceAsync(IQueryable<T> query, int page, int quantity)
	{
		page = page == 0 ? 1 : page;
		quantity = quantity == 0 ? 10 : quantity;

		int total = await query.CountAsync();
		List<T> items = await query.Skip((page - 1) * quantity).Take(quantity).ToListAsync();
